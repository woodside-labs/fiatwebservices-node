...
import { APIPromise } from '@fiatwebservices/sdk/api-promise';

import util from 'node:util';
import Fiatwebservices from '@fiatwebservices/sdk';
import { APIUserAbortError } from '@fiatwebservices/sdk';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['FIATWEBSERVICES_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Fiatwebservices) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new Fiatwebservices({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'FIATWEBSERVICES_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['FIATWEBSERVICES_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Fiatwebservices({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['FIATWEBSERVICES_BASE_URL'] = 'https://example.com/from_env';
      const client = new Fiatwebservices({ apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Fiatwebservices({ maxRetries: 4, apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['FWS_API_KEY'] = 'My API Key';
    const client = new Fiatwebservices();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['FWS_API_KEY'] = 'another My API Key';
    const client = new Fiatwebservices({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Fiatwebservices({ apiKey: 'My API Key', fetch: testFetch });
